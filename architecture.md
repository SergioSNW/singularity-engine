...
> **Current version:** v0.20.0-alpha — Workspace Layout & Panel Organization: `LayoutManager` is reborn as **`WorkspaceManager`** (`src/core/WorkspaceManager.{h,cpp}`), which owns the full-screen DockBuilder node tree and three task-oriented **workspaces**: **Level Design** (Hierarchy over Stats on the left, Viewport center, Inspector + Editor Settings tabbed in the right rail, and a bottom Development Zone tabbing Content Browser + Console next to the Script Editor), **Scripting** (taller bottom IDE strip: script sidebar + docked code window + Content Browser/Console tabs), and **Shading & Assets** (wide right rail of Inspector + Settings + Content Browser tabs, bottom Console + Stats tab group, maximized viewport). Tab groups render several panels into one tabbed dock window to conserve real estate. The main menu bar gains a **Workspace** selector dropdown (radio checkmarks per workspace + Reset + Save Current Layout as Default), the command palette's "Layout" commands move to the "Workspace" category with a new Shading & Assets entry, and `editor_layout.json` now stores the active `workspace` key (with a legacy `preset` fallback for v0.18/0.19 files). Materials (Phase 19), grid snapping and duplication (Phase 18), and the Phase 17 scene graph are unchanged.
