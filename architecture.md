...
> **Current version:** v0.28.0-alpha — Final UI & Workspace Consolidation: the editor finishes with a status bar, toast notifications, a viewport RMB context menu, and a consolidated workspace layer. **Status bar** (`Application::DrawStatusBar`, reserved as a 24 px strip at the bottom of every workspace via `WorkspaceManager::SetBottomBarHeight`, toggled from the View menu) shows frame time/FPS (EMA-smoothed in `Application::Run`), entity count, audio channels in use (`AudioManager::ActiveChannelCount`), enabled viewports (`CameraManager::EnabledCount`), and the current workspace name (`WorkspaceManager::WorkspaceName`). **Toasts** (`ToastManager`, a new pure `src/core/ToastManager.{h,cpp}` class) overlay top-right: timed notifications that refresh a repeated newest message, cap at 5 with oldest eviction, and fade out over the final 400 ms — fed by save/load/new/play/stop/duplicate/delete/import events. **Viewport RMB context menu**: a quick right-click on the viewport (under the fly-mode drag threshold) opens a "Viewport Context" popup with Rename (inline undoable modal), Duplicate, Delete, and Create (Empty Entity, Cube, Octahedron, Directional Light, Camera — all undoable, spawning at the editor camera), while a drag still enters fly navigation; the palette shortcut becomes `Ctrl+P` (with F1 / `Ctrl+Shift+P`). **Hierarchy rename** (`DrawRenameRow`) and **Content Browser Duplicate** (`DuplicateAsset`, `<stem>_copy` with numeric collision suffixes, recursive folder copy) round out the asset workflow; workspaces gain Reset-to-Default and Save-Current-as-Default. The multi-viewport camera stack, audio, lighting, and free-fly camera from v0.24.0–v0.27.0 are unchanged.
