...
> **Current version:** v0.24.0-alpha — Directional Lighting, Shadows & Environment Controls: the shading pipeline is now driven by scene-authored `DirectionalLightComponent`s (light color, intensity, travel direction, ambient floor) instead of a hardcoded light vector, with a ray-cast directional shadow that attenuates over distance (`shadow_strength` / `shadow_bias` / `shadow_distance`). Every scene — the default map, `New Scene`, and loaded files — is guaranteed at least one active light (`CreateDirectionalLightEntity` / `EnsureActiveLight`), light properties are editable in the Inspector (undoable through the CommandHistory snapshot), new lights are created from the Hierarchy's Scene Root context menu, and the Level Design workspace gives the Hierarchy a full-height rail beside the dev-zone tools. Prefab drop-instantiation crash hardening from v0.23.1 is unchanged.
