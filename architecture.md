...
> **Current version:** v0.22.0-alpha — Advanced UI Polish & Workflow Suite: the editor gains a **custom Roboto UI font** (bundled `assets/fonts/Roboto-*.ttf`, Segoe UI/Arial fallback), a **global Command-pattern Undo/Redo** system (`src/core/CommandHistory.{h,cpp}`, `Ctrl+Z`/`Ctrl+Y`/`Ctrl+Shift+Z`, Edit menu, 100-step stacks) covering gizmo transform drags, Inspector property edits (one undo step per gesture), entity create/duplicate/delete/re-parent, and prefab spawns — with whole-entity before/after snapshots and byte-for-byte subtree restore via new `SceneSerializer::SerializeEntityTree`/`SpawnEntityTree` — plus a dockable **History panel** listing both stacks. The **Content Browser** shows live image thumbnails for textures and diffuse-color swatches for `.mat` assets, and the **Workspace menu** is grouped into Workspace Presets (active checkmark) and Layout sections. The Material Editor, workspaces, materials, grid snapping, and duplication from earlier phases are unchanged.
