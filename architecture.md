...
> **Current version:** v0.25.0-alpha — Free-Fly Editor Camera & Viewport Navigation: the editor now owns a dedicated free-fly camera (`EditorCamera` in `src/core/EditorCamera.h`) independent from the scene's gameplay camera entities. Fly Mode — right-click while hovering the viewport — captures the cursor and drives the editor camera with WASD/QE movement and FPS-style mouse look, and the Fly Speed / Rotation Sensitivity sliders in the Editor Settings panel tune it (`EditorCameraSettings`). Toggling Play Mode smoothly blends the view between the editor camera and the active gameplay camera over `kCameraTransitionDuration` (0.6 s) with a pure, harness-tested smoothstep `CameraBlend` that interpolates position/pitch/yaw/fov and turns yaw along the shortest arc. During play the viewport follows the gameplay camera entity; on Stop it glides back to the editor camera. Directional lighting, shadows and the environment controls from v0.24.0 are unchanged.
