...
> **Current version:** v0.19.0-alpha — Basic Material System & Texture Support: materials are first-class `.mat` JSON assets (`Material` struct + `MaterialLibrary` cache under `assets/materials/`) carrying an RGBA albedo tint, an optional texture filename, and a `shininess` knob; `stb_image.h` (vendored under `third_party/stb/`, `STB_IMAGE_IMPLEMENTATION` in `Texture.cpp`) decodes images that `TextureLibrary` uploads as cached `SDL_Texture`s with linear filtering and alpha blending. The render pass stays SDL2 `SDL_RenderGeometry` but `Mesh` now carries per-vertex UVs (built-in cube + OBJ `vt` parsing with v-flip, empty-UV fallback to flat shading) and `FillTri`/`DrawTriangles` batch textured triangles, with `Application::ResolveEntityTexture` resolving an assigned `.mat` asset over the entity's own color/texture. The Inspector's Material section gains a `.mat` asset combo, a **New Material** creator, a texture combo with live `ImGui::Image` preview, and drag-drop assignment (`MATERIAL`/`TEXTURE` payloads from the Content Browser, which classifies `.mat` and image files as new `FileKind`s). `MaterialComponent` persists `material_path`/`texture_path` in the scene JSON. The scene graph from Phase 17 and grid snapping from Phase 18 (`SnapSettings`, Ctrl+D duplication) are unchanged.
