...
> **Current version:** v0.38.0-alpha — Dedicated Material Authoring & Shading Mode: materials graduate from a color tint + single texture into an explicit **PBR channel layout**. Each `.mat` asset (round-tripped through the `json::Value` serializer like `.env`/`.pmat`) now carries **Albedo** (RGBA tint + albedo map + `albedo_multiplier`), a **Normal** slot (`normal_texture`/`normal_strength`, documented slot-only in the software rasterizer), and per-channel scalars + optional texture-map slots + multipliers for **Metallic**, **Roughness** and **Ambient Occlusion**. The **software shade loop** (`EmitEntityTris`) consumes the resolved scalars (`MaterialShading`, pulled from the assigned `.mat` by the new `ResolveEntityShading`): the multiplier scales the albedo, AO dims the ambient floor (`pbr::AmbientFloor`), and metallic/roughness drive a cheap **Blinn-Phong specular** term — `pbr::SpecularPower` (roughness 0 = 257-power mirror, 1 = dead matte), `pbr::DielectricF0` (dielectrics at 4%, metals at their albedo), `pbr::BlinnPhong` — via `src/render/MaterialCore.h`, a pure header verified by a standalone g++ harness. The editor surfaces it all: the **Material Editor** gains PBR sections (texture-slot combos, multipliers, collapsible channel groups) with **live updates** — `MaterialLibrary::LiveUpdate` pushes every edit into the cache in memory so the scene and the preview re-shade instantly, while **Save** persists the `.mat`; a **Create New Material wizard** (also from the Command Palette / workspace button) authors a new asset from name + albedo + metallic/roughness. A dedicated **Material Preview viewport** (`MaterialPreviewPanel` + `Application::RenderMaterialPreview`) renders a procedural **UV sphere or cylinder** under the active environment settings (sky + fog + post) and the scene lights (with a key-light fallback), orbit-controlled (drag/scroll/auto-rotate), docked beneath the main viewport in the Shading & Assets workspace.
