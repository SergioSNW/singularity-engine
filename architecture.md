...
> **Current version:** v0.23.0-alpha — OS File Drop Ingestion & Viewport Drag-Drop Spawning: files dropped onto the editor window from the OS file manager are **imported into the assets tree** by a new dependency-free `AssetImporter` module (`src/core/AssetImporter.{h,cpp}` — extension-classified sub-folders, normalized targets, collision suffixes), instantly visible in the Content Browser via auto-refresh plus a transient "Imported N file(s)" overlay. The **viewport** is now a first-class drop target (`BeginDragDropTargetCustom` over its image rect): prefab/mesh assets drop at the cursor's `y=0` ground point (spawned, selected, undoable via `PushSpawn`), and material/texture assets assign to the current selection as undo transactions. **Hierarchy** rows, the Scene Root header, and empty tree space accept mesh/material/texture drops too (`InstantiateMeshAsset` spawns mesh children; assigns are undo-aware), and Content Browser mesh assets drag with a `"MESH"` payload. Undo/redo, the History panel, Roboto font, workspaces, materials, grid snapping, and duplication from earlier phases are unchanged.
