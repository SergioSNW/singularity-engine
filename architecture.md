...
> **Current version:** v0.16.1-alpha — Play Mode Isolation & Window Scaling: entering play mode hides the non-essential editor windows (Script Editor, Content Browser, Console, Inspector) via `Application::SavePlayModePanelState()` so the isolated viewport renders a clean game view; `ExitPlayMode` restores every panel to its exact pre-play visibility before the dock layout rebuild. The play render branch now draws only the viewport (the Script Editor's floating code window is no longer overlaid on the game view). The primary window grows from 1280×720 to **1580×1020** (+300px each) in `src/core/main.cpp` for a roomier dockspace.
