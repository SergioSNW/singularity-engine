...
> **Current version:** v0.34.0-alpha — Landscape & Topology Design Suite: the engine gains its first **procedural terrain** pipeline. A new **`LandscapeComponent`** stores a resolution/`size`-parameterized heightfield — `resolution × resolution` quads with per-vertex heights (`std::vector<float>`), a runtime-only generated `Mesh`, and a dirty flag — and a new **`Landscape`** core module builds that mesh (grid geometry with +Y normals, sparse surface-riding wireframe edges, AABB bounds) and provides the sculpt kernels: **Raise** (smoothstep-falloff elevation), **Smooth** (relax toward neighbor average), and **Flatten** (blend to the bilinear height at the brush center), all operating in the landscape's local space with a 4×4 affine inverse so rotated/scaled terrains sculpt correctly. A new **Landscape Mode** workspace preset replaces the transform gizmo in the viewport with a projected **brush-sphere cursor** tracking the mouse on the terrain, driven by a new `LandscapePanel` (brush size/strength/falloff sliders + tool palette + create button); strokes are stamped per frame (`strength × dt`), bracketed by a single **"Sculpt Landscape"** undo transaction per stroke, and the heightfield round-trips through `SceneSerializer` and command snapshots. Entity AABBs, picking (gizmo ray-cast) and the render passes all route through a new `ResolveEntityMesh` so landscapes and mesh-library entities share one pipeline.
