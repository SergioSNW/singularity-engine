...
> **Current version:** v0.35.0-alpha — Animation & Timeline Foundation: the editor gains its first **keyframe animation** pipeline. A new **`AnimationComponent`** on `Entity` stores three transform-property tracks — **Position**, **Rotation** and **Scale** — each a time-sorted `AnimationTrack` of `AnimationKeyframe { time, value[3] }` (Euler degrees for rotation), with a per-entity `loop` flag and a `duration` that mirrors the longest key time. A lean **`Anim`** core module (`src/core/Animation.{h,cpp}`) provides the sampling: **LERP** for position/scale, **SLERP** in quaternion space for rotation (shortest-arc, with stored Euler reproduced verbatim at exact keyframe times so recorded poses never drift through the ±180° ambiguity), plus `SetKeyframe`/`RemoveKeyframe`/`TrackDuration`. The Application owns a global **`TimelineState`** clock (time/duration/playing/loop) and a shared **`TimelineBridge`** that the new track-based **Sequencing workspace** (`Workspace::Timeline`) — which replaces the viewport center-stage with a `TimelinePanel` (transport controls, scrub slider, per-property lanes with keyframe diamonds and playhead) — and the Inspector's keyframe toggles (amber while a key sits at the playhead) both fire into. Playback advances the clock each editor frame and writes sampled poses to every animated entity, gated so paused scrubs never stomp gizmo/Inspector edits; recording keys is undoable, stretches the global duration, and the tracks round-trip through `SceneSerializer` and command snapshots.
