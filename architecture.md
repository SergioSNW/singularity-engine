...
> **Current version:** v0.36.0-alpha — Physics Materials & Collision Layer Matrix: physics is no longer a single uniform interaction. A **`PhysicsMaterial`** asset (`.pmat`, JSON under `assets/physics/`, managed by a `PhysicsMaterialLibrary` mirroring `MaterialLibrary`) pairs a **friction** (0–1 tangential grip) with a **restitution** (0–1 bounciness) and is created, saved and assigned to any collider straight from the Inspector's Collider section (with the pair combined via the standard rules — restitution = max, friction = geometric mean). A **bitmask collision layer system** replaces the old every-body-collides model: the scene owns a `CollisionMatrix` of **16 named layers**, each collider carries a **membership bitmask** (multiple layers per body), and two bodies now interact during the physics step **only when the matrix allows any of their layers to collide** — a symmetric grid the new **Collision Matrix panel** (docked first in every workspace's shared tab groups) edits live, with inline layer renaming, self-collision control and a "Reset All Pairs" action. Layer bitmasks, material assignments and the whole matrix serialize with the scene file, ride the undo snapshots, and gate both solid resolution and trigger events, so scenes tuneable by layer (Player vs. Environment, Projectile vs. Enemy, projectiles that ignore each other) keep working exactly as authored.
