...
> **Current version:** v0.33.0-alpha — True Workspace Layouts, Tabbed Mini-IDE & Theme: the **WorkspaceManager** becomes fully state-driven — the built-in workspaces (Level Design, Scripting, Shading & Assets) now describe canonical dock layouts for the unified **Script Editor** mini-IDE, and a new **Reset to Workspace Default** action rebuilds the active workspace's canonical layout (forgetting any captured custom layout). The **ScriptEditorPanel** is reworked into a **tabbed mini-IDE**: multiple `.lua` scripts stay open at once, each tab owning its own `TextEditor` buffer, undo stack, and saved-on-disk baseline (dirty tabs carry a `*`, and closing one confirms unsaved changes); a toolbar **Float/Dock to Workspace** toggle pops the whole window out of the dock or back in, and the code pane gets a distinct **mini-theme** with a contrasting line-number gutter (a new `PaletteIndex::LineNumberFill` in the vendored TextEditor). The Lua REPL, auto-save/hot-reload hooks from v0.32.0-alpha and all viewport/camera/lighting/audio features from v0.24.0–v0.31.0 are unchanged.
