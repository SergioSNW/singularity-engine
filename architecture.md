...
> **Current version:** v0.16.0-alpha — Console Redirection & Workflow Layout: a new `Console` sink (`src/core/Console.{h,cpp}`) becomes the engine's single log destination — Lua `print()` is overridden to funnel through it as Info, `ScriptEngine` routes bind failures and runtime exceptions into it as Error (deduplicated so a persistent error logs once, not 60×/s), and C-level `stdout`/`stderr` (printf, `std::cout`, any stray `fprintf`) are captured through two OS pipes (`CreatePipe`/`SetStdHandle`/`_dup2`, drained non-blocking each frame via `PeekNamedPipe`) so an external terminal is no longer needed. The dockable **Console Panel** (`src/editor/ConsolePanel.{h,cpp}`) renders the buffer color-coded (Info white, Warning yellow, Error red) with a Clear button and an Auto-scroll toggle. Workspace ergonomics: the default layout now reserves a bottom "Development Zone" strip of Content Browser | Script Editor | Stats (Scripting preset: sidebar | docked code window | Content Browser over Stats), with the Console docked under the Hierarchy on the left rail in both presets.
