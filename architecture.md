...
> **Current version:** v0.15.0-alpha — Scene Management, Prefabs & Content Browser: a new `SceneManager` (`src/core/SceneManager.{h,cpp}`) owns the single active `Scene` (allocated once, rebuilt in place so every `Scene*` stays valid) and drives file-backed transitions (`LoadScene`/`SaveScene`/`NewScene`). Scene files gained a `"meta"` block (`name`/`author`/`created`) stamped on save. Prefabs serialize a single entity tree (root + `"children"`) into `{ "prefab": true, ... }` `.prefab.json` files via `SceneSerializer::SavePrefab`/`LoadPrefab`, instantiating fresh UUIDs on every spawn. A dockable **Content Browser** (`src/editor/ContentBrowserPanel.{h,cpp}`) browses `assets/` (recursive folder tree + responsive file grid with per-type badges), provides create-folder/rename/delete, double-click loads scenes, opens `.lua` in the Script Editor, and spawns prefabs — which can also be dragged from the grid onto the Hierarchy window or spawned from a prefab picker. Map metadata round-trips through JSON, and scenes are switched in-place, so panels, physics, and scripts keep valid pointers across a map change.
