...
> **Current version:** v0.30.0-alpha — Real-Time Performance Profiler UI: the engine gains a lightweight, dependency-free telemetry core — `Profiler` (`src/core/Profiler.h`) — with a 120-frame rolling ring buffer per stage (Update / Render / UI / Physics) plus a live resource snapshot (entity count, 3D draw calls, resident memory). `Application::Run()` brackets every phase each frame: the Update stage wraps gameplay scripts + editor gizmo interaction (physics steps nested inside as a dedicated stage), the Render stage wraps the off-screen 3D pass + Inspector camera preview, and the UI stage spans ImGui's NewFrame→Render plus the final SDL blit/Present. `RenderScenePass` / `RenderEditorOverlay` now thread an `int &draw_calls` tally through every `SDL_RenderDrawLine` / `SDL_RenderGeometry` call so the panel shows real 3D draw-call load. A new dockable **ProfilerPanel** (`src/editor/ProfilerPanel.{h,cpp}`) plots the trends (frame time + per-stage ms, entities, draw calls, memory MB) with latest/avg/peak readouts, a **Pause/Resume** button that freezes the buffers for frame inspection, and Clear; it is toggled from the View menu and the command palette (`Toggle Profiler`, `Pause/Resume Profiler`, `Clear Profiler Data`). Resource memory uses new `MeshLibrary::ResidentBytes()` / `TextureLibrary::ResidentBytes()` estimates plus the live entity count. The Profiler core is pure (no SDL/ImGui) and verified by the standalone `phase30_profiler_test` harness. Viewport overlays, workspace persistence, multi-viewport camera stack, lighting, and audio from v0.24.0–v0.29.0 are unchanged.
