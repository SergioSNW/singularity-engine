...
> **Current version:** v0.21.0-alpha — Material Editor Panel & UI Modernization: a dedicated dockable **`MaterialPanel`** (`src/editor/MaterialPanel.{h,cpp}`) turns `.mat` assets into an authorable surface: a two-pane editor (asset list left, properties right) with diffuse **color tint**, **texture slot** selection, a **live `ImGui::Image` preview**, and a shininess knob. Edits go to a working copy and are persisted via a new `MaterialLibrary::Save()` (`src/core/Material.cpp`) that rewrites the `.mat` file and refreshes every cached copy so meshes re-tint on the next render without restarting; new white materials are created from the panel. It is docked as the primary right-rail **authoring zone** of the **Shading & Assets** workspace (with Inspector + Editor Settings + Content Browser tabbed beneath it) and as a tab in the other workspaces' development zone, plus a command-palette entry and View-menu toggle. The ImGui style gets a **Unity/UE-style pass** in `Theme::ConfigureStyle`: breathable padding/spacing (window padding 10→16), softened rounding (window 8→12, frames 4→6, tabs 4→8), toned-down borders (windows/children/tabs now borderless), a clearer studio-blue accent (0x4D8DFF), accent-warmed button hovers, stronger tab/selection tinting, and a `Theme::PushPrimaryButtonColor()` helper that accents the Save/Create actions. Materials (Phase 19), workspaces (Phase 20), grid snapping and duplication (Phase 18), and the Phase 17 scene graph are unchanged.
