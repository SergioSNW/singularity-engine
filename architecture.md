...
> **Current version:** v0.13.0-alpha — Modern UI theme & master dockspace layouts: a dedicated `Theme` module (`src/editor/Theme.{h,cpp}`) replaces stock grey with a professional dark palette (warm charcoal neutrals, indigo accent, refined rounding, `TabBarOverlineSize` selected-tab highlight) and makes text razor-sharp on high-DPI displays by rasterizing Segoe UI / Segoe UI Semibold / Cascadia Mono at the display's framebuffer scale and folding that factor out of `FontGlobalScale`. A new `LayoutManager` (`src/core/LayoutManager.{h,cpp}`) owns a single full-screen transparent `DockSpace` host window so every panel — including the script IDE's code window, which can now be docked via `SetNextWindowDockID` — lives in one unified workspace; the View → Layout menu switches Default vs Scripting presets, resets to the pristine Default workspace, or captures the current arrangement as the startup default (`editor_layout.json`). Script editing keeps `Save & Reload` hot-swap through `ScriptEngine::ReloadSession`, and script paths round-trip through the scene serializer via `ScriptComponent.path`.
