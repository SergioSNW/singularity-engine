...
> **Current version:** v0.17.0-alpha — Hierarchical Scene Graph (Parent-Child Transforms): entities form a scene graph via `Entity *parent` / `std::vector<Entity*> children` links (stable under `unique_ptr` ownership), with `Scene::ComputeWorldMatrix()` folding a child's local `Mat4TRS` through every ancestor (`World = ParentWorld * Local`). `Scene::SetParent()` maintains both links, detaches on `-1`, and rejects self-parenting, reparenting into the entity's own subtree (cycles), and unknown targets **before** unlinking — a rejected reparent is now a strict no-op that leaves the hierarchy untouched. The Hierarchy panel renders the tree with indentation, and Phase 17 adds **drag-and-drop re-parenting**: dragging an entity row onto another entity parents it there, while dropping on Scene Root or empty space detaches it to the root (dropping onto a node in the dragged entity's own subtree is not offered, so cycles cannot be created from the UI).
