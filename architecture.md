...
> **Current version:** v0.13.1-alpha — Contextual inspector, global command palette & live theme customizer: the `Theme` module now exposes six user-editable color tokens (`Theme::Colors`: window/child/popup/frame backgrounds, text, accent) and derives every other style color (borders, hovers, tabs, scrollbars, focus rings) from them, so the whole editor re-skins coherently when a token changes. A new `SettingsPanel` ("Theme Settings", View → Theme Customizer) live-edits those tokens and persists them to `editor_theme.json` (gitignored) via `Theme::SaveThemeToFile` / `LoadThemeFromFile`. The `InspectorPanel` was refactored into clean collapsible component headers with subtle ghost action buttons (Reset / Reset to Cube / Clear) and a minimal centered placeholder when nothing is selected. A new `CommandPalette` (`Ctrl+Shift+P`) fuzzy-matches core editor actions (open script editor, toggle theme customizer, switch/reset layouts, save/open scene, enter play mode) in a modal with instant keyboard focus, Up/Down navigation, and Enter/click execution.
