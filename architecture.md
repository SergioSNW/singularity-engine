...
> **Current version:** v0.27.0-alpha — Camera & Multi-Viewport Management: the engine's single-camera assumption is replaced by a `CameraManager` (`src/core/CameraManager.h` / `.cpp`), a pure, headless-testable stack of camera entries that couples a camera *source* (the free-fly editor camera / play-mode blend, or a scene entity's `CameraComponent`) to a *viewport layout definition* (normalized rect + z-order + primary flag). The viewport renderer is now a multi-pass renderer: it walks the stack bottom-up by z-order and draws each enabled entry into its own region of the shared supersampled target, so split-screen / multi-camera scenes render in one pass pipeline. Exactly one entry is primary: it owns editor input (gizmo picking, asset drops), near/far picking, and the editor overlays (selection, bounds boxes, colliders, gizmo handles). A new **Viewport Layout** panel (`src/editor/ViewportLayoutPanel.h` / `.cpp`, docked in the dev zone, View menu + command palette) edits the stack live — source combo, normalized rect, z-order, enabled, primary radio, add/remove, reset. The Inspector's Camera section gained a live **Camera Preview**: the selected camera entity is rendered into a small off-screen target each editor frame and shown as an image. The default layout is the classic single full-screen viewport, so the free-fly camera, transition, lighting, and audio from v0.24.0/v0.25.0/v0.26.0 are unchanged.
