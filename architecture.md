...
> **Current version:** v0.23.1-alpha — Prefab Drop-Instantiation Hotfix: dropping prefab/mesh assets onto the Viewport or Hierarchy no longer crashes the editor. Entity-creating drops on hierarchy rows are **deferred until after the entity-tree iteration** (`FlushPendingSpawns`), closing a vector-reallocation iterator-invalidation crash in `Scene::CreateEntity`; every drop payload is null/size-guarded before any scene mutation, hostile/empty drop entries are ignored, and both the JSON parser (`kMaxJsonDepth`) and the tree deserializer (`kMaxTreeDepth` in `SceneSerializer`) cap nesting so a malformed or adversarial prefab is rejected cleanly instead of overflowing the stack. OS file-drop ingestion, viewport spawn/assign drops, and the Hierarchy/Content Browser drop extensions from v0.23.0 are unchanged.
