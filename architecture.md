...
> **Current version:** v0.14.0-alpha — Physics-Scripting Bridge: a new `ColliderComponent` (local box `center +/- extents`, `Solid` or `Trigger`, default disabled) gives entities a physics volume. A new `PhysicsManager` (`src/core/PhysicsManager.{h,cpp}`) runs an AABB broad-phase pass in play mode: **Solid vs Solid** pairs are penetration-prevented by separating the higher-id body along the minimum-penetration axis, while **Trigger** volumes are pass-through and raise events only. Overlaps are edge-tracked in a per-pair map so Lua hooks (`OnCollisionEnter/Exit`, `OnTriggerEnter/Exit(other)`) fire exactly once per transition, never per-frame; the hooks are bound by `ScriptEngine::DispatchEvent`, which passes the other entity as a Lua `Entity` handle. `Application` drives `UpdateSession(dt)` then `PhysicsManager::Step` each play frame; the Inspector gained a Collider component section and the editor viewport draws collider volumes (green solid / cyan trigger). A Wall + Bouncer + Trigger Zone demo scene demonstrates solid stopping and trigger pass-through.
