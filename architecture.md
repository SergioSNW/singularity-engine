...
> **Current version:** v0.26.0-alpha — Audio & Sound Effects: the engine now plays sound through SDL_mixer 2.8.2 (built statically like SDL2, only the dependency-free codecs enabled: WAV + stb_vorbis OGG). `AudioManager` in `src/core/AudioManager.h` / `.cpp` opens a 44.1 kHz stereo device on a 16-channel pool, lazily loads and caches samples, and exposes `Play(path, volume, loop)` / `Stop(path)` / `StopAll()` plus a master-volume gain; it is optional at runtime, degrading to a silent no-op when no audio device can be opened. Entities carry an `AudioComponent` (`path`/`loop`/`volume`/`auto_play`) that persists in scene files, is undoable, and can auto-play on entering play mode; the Lua bridge exposes `Audio.Play`/`Audio.Stop`, the Inspector gained an Audio section with preview buttons, and the AssetImporter routes `.wav`/`.ogg` into `assets/audio/`. The free-fly editor camera, transition, and lighting from v0.24.0/v0.25.0 are unchanged.
