...
> **Current version:** v0.32.0-alpha — Integrated Lua Scripting IDE: the script subsystem becomes a full authoring surface. **ScriptEditorPanel** (the embedded ImGuiColorTextEdit-based Lua editor) gains real-time editing hooks: an **Auto-save** toggle saves the buffer the moment the code window loses focus, and a disk watcher detects external edits — adopting the new text and hot-reloading the running play session via the existing `ScriptEngine::ReloadSession` callback, never clobbering a dirty buffer. The **ConsolePanel** gains a **Lua REPL command line**: typing a snippet and pressing Enter fires a new `ScriptEngine::Execute()` method, which evaluates live Lua against the active scene in a persistent, isolated scratchpad VM (`Singe.EngineApi` chained `_ENV` with a `scene` table exposing `count()` / `get(i)` / `find(name)` / `name`), routing `print()` output, chunk return values (`=> ...`), and compile/runtime errors into the shared Console sink. The REPL state outlives play sessions, so definitions persist across the whole editor run. Profiler telemetry from v0.30.0-alpha and all viewport/camera/lighting/audio features from v0.24.0–v0.29.0 are unchanged.
